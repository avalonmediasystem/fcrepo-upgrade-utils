...
public final class RdfUtil {
    ...
    public static InputStream writeRdfTranslateIds(final Model rdf,
                                                   ...
                                                   final List<String> excludedFromSystemProperties) {
        try (final var baos = new ByteArrayOutputStream()) {
            final var writer = StreamRDFWriter.getWriterStream(baos, lang);
            writer.start();

	    Predicate<Statement> dropStatement = (s) -> (RdfUtil.isServerManagedTriple(s) && !excludedFromSystemProperties.contains(s.getPredicate().toString()));
